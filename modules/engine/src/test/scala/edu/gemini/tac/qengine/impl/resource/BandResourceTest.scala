...
//   private def mkProp(time: Time): Proposal =
//     Proposal(ntac.copy(awardedTime = time), site = Site.GS, obsList = List(Observation(null, target, conds(IQAny), time)))

//   private def mkProp(iq: ImageQuality, b3IQ: Option[ImageQuality]): Proposal = {
//     val b3 = List.empty
//     Proposal(ntac, site = Site.GS, band3Observations = b3, obsList = List(Observation(null, target, conds(iq), Time.hours(10))))
//   }

//   private def mkProp(too: Too.Value): Proposal =
//     Proposal(ntac, site = Site.GS, too = too, obsList = List(Observation(null, target, conds(IQAny), Time.hours(10))))

//   private def mkProp(lgs: Boolean): Proposal =
//     Proposal(ntac, site = Site.GS, obsList = List(Observation(null, target, conds(IQAny), Time.hours(10), lgs)))

//   val br      = new BandResource(Default.BandRestrictions)
//   val timeMap: Map[Partner,Time] = Map(US -> Time.hours(100))
//   val queue   = ProposalQueueBuilder(Fixture.evenQueueTime(100, Some(QueueTime.DefaultPartnerOverfillAllowance)))
...
//     // Rapid TOO Band 1
//     val prop1 = Proposal(copyNtac("us1", 30), site = Site.GS, too     = Too.rapid)
//     // LGS Band 2
//     val prop2 = Proposal(copyNtac("us2", 15), site = Site.GS, obsList = obsList(IQ70, lgs = true))
//     // IQ20 Band 2
//     val prop3 = Proposal(copyNtac("us3", 15), site = Site.GS, obsList = obsList(IQ20, lgs = false))
//     // Nothing, Band 3
//     val prop4 = Proposal(copyNtac("us4", 40), site = Site.GS, band3Observations   = obsList(ImageQuality.IQ70, lgs = false, CCAny))
...
//     val propX = Proposal(copyNtac("usX",  1), site = Site.GS, too     = Too.rapid)
//     // LGS Band 2
//     val prop2 = Proposal(copyNtac("us2", 15), site = Site.GS, obsList = obsList(IQ70, lgs = true))
//     // IQ20 Band 2
//     val prop3 = Proposal(copyNtac("us3", 15), site = Site.GS, obsList = obsList(IQ20, lgs = false))
//     // Nothing, Band 3
//     val prop4 = Proposal(copyNtac("us4", 39), site = Site.GS, band3Observations   = obsList(IQ70, lgs = false))
...
//     val prop1 = Proposal(copyNtac("us1", 30), site = Site.GS, too     = Too.rapid)
//     // Rapid TOO Band 2 -- will be removed
//     val propX = Proposal(copyNtac("usX", 15), site = Site.GS, too     = Too.rapid)
//     // LGS Band 2
//     val prop2 = Proposal(copyNtac("us2", 15), site = Site.GS, obsList = obsList(IQ70, lgs = true))

//     // IQ20 Band 3 -- but will be moved up 15 hours and back into band 2 when
//     // propX is removed.
//     val prop3 = Proposal(copyNtac("us3", 15), site = Site.GS, band3Observations = obsList(IQ20, lgs = false), obsList = obsList(IQ20, lgs = false))

//     // Nothing, Band 3
//     val prop4 = Proposal(copyNtac("us4", 25), site = Site.GS, band3Observations = obsList(IQ70, lgs = false))
...
//   //   val prop2 = Proposal(copyNtac("us2", 29), site = Site.GS, band3Observations = obsList(IQAny, lgs = false))

//   //   // Now, the last band 2 proposal, will be one that doesn't accept band 3.
//   //   val propX = Proposal(copyNtac("usX", 2), site = Site.GS)

//   //   // The remainder of the joint which also includes prop1
//   //   val prop3 = Proposal(copyNtac("us3", 15), site = Site.GS, band3Observations = obsList(IQAny, lgs = false))
