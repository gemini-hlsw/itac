...
import scalaz._
import Scalaz._
import edu.gemini.model.p1.immutable.`package`.InvestigatorGender
...
class ProposalFixture {
  ...
  def tacCategory = Some(im.TacCategory.GALAXY_EVOLUTION)
  ...
  def pi =
    im.PrincipalInvestigator(
      piUuid,
      "Biff",
      "Henderson",
      Nil,
      "bhenderson@bovine.edu",
      piStatus,
      InvestigatorGender.NONE_SELECTED,
      piAddress
    )
  ...
  def siderealCoords =
    Coordinates(
      RightAscension.fromAngle(Angle.parseHMS("10:11:12").getOrElse(Angle.zero)),
      Declination
        .fromAngle(Angle.parseDMS("-20:30:40").getOrElse(Angle.zero))
        .getOrElse(Declination.zero)
    )
  val targetUuid = UUID.randomUUID()
  def siderealTarget =
    im.SiderealTarget(targetUuid, "BiffStar", siderealCoords, im.CoordinatesEpoch.J_2000, None, Nil)
