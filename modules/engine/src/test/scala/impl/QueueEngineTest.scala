...
class QueueEngineTest {
  ...
  def coreProposal(
    ...
    awardedHours: Double,
    site: Site,
    os: List[Observation] = List.empty,
    isPoorWeather: Boolean = false,
    mode: Mode = Mode.Queue,
    b3s: List[Observation]
  ) = {
  ...
  def randomOs(i: Int, cc: Option[CloudCover] = None): List[Observation] =
    (for (j <- 1 to i) yield randomO(cc)).toList
  ...
  def validateInRankOrder(ps: List[Proposal]) =
    partners.map { partner =>
      ps.filter(p => p.ntac.partner == partner)
        .foldLeft(-1.0)((lastRank, currentProposal) => {
          val myRank = currentProposal.ntac.ranking.num.get
          Assert.assertTrue(
            "%s rank %f found after rank %f".format(partner.toString, myRank, lastRank),
            myRank >= lastRank
          )
          myRank
        })
