...
class QueueCalcTest {
  ...
  @Test def testLogUpdate() {
    ...
    val propUS2 =
      Fixture.mkProp(Ntac(US, "US2", 2, Time.hours(0.5)), (target0, Fixture.badCC, Time.hours(0.5)))
  ...
  @Test def testStopWhenChangeTimeCategory() {
    ...
    val propGS1 = Fixture
      .mkProp(Ntac(GS, "GS1", 1, Time.hours(0.5)), (target23, Fixture.badCC, Time.hours(0.2)))
      .copy(band3Observations =
        List(new Observation(target23, ObservingConditions.AnyConditions, Time.hours(1)))
      )

    // Create a proposal to take us past band 3.
    val propCA1 = Fixture
      .mkProp(
        Ntac(CA, "CA1", 1, Time.hours(qstate.queueTime.bandPercentages.band3 * hrs)),
        (target23, Fixture.badCC, Time.hours(0.2))
      )
      .copy(band3Observations =
        List(new Observation(target23, ObservingConditions.AnyConditions, Time.hours(1)))
    ...
    val propAU1 = Fixture
      .mkProp(Ntac(AU, "AU1", 1, Time.hours(0.2)), (target23, Fixture.badCC, Time.hours(0.2)))
      .copy(band3Observations =
        List(new Observation(target23, ObservingConditions.AnyConditions, Time.hours(1)))
      )
    ...
    val propLists: Map[Partner, List[Proposal]] = Map(
      US -> List(propUS1),
      BR -> List(propBR1),
      GS -> List(propGS1),
      CA -> List(propCA1),
    ...
    val blit =
      BlockIterator(All, Fixture.genQuanta(20.0), List(US, BR, GS, CA, AU), propLists, _.obsList)
    ...
    val params3 = new QueueCalcStage.Params(
      B3,
      calc.queue,
      calc.iter,
      _.band3Observations,
    ...
    calc3.queue.bandedQueue.keys.map { band =>
  ...
  @Test def testRollback() {
    ...
    val propLists: Map[Partner, List[Proposal]] =
      Map(US -> List(propUS1), BR -> List(propBR1), GS -> List(propGS1))
    val blit =
      BlockIterator(All, Fixture.genQuanta(5.0), List(US, BR, GS, BR), propLists, _.obsList)
