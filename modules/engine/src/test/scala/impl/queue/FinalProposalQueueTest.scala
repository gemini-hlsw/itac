...
class FinalProposalQueueTest {
  ...
  val delta     = 0.000001
  val site      = Site.GS
  val queueTime = new QueueTime(site, PartnerTimes.distribute(Time.hours(100), site, partners))
  ...
  @Test def testSingleProposal() {
    ...
    verifyTime(
      queueTime(Category.Guaranteed).toHours.value - 1,
      queue.remainingTime(Category.Guaranteed)
    )
  }
  ...
  @Test def testMultipleProposals() {
    ...
    verifyTime(
      queueTime(Category.Guaranteed).toHours.value - 4,
      queue.remainingTime(Category.Guaranteed)
    ) // 4 hrs band 1,2,3
  }
  ...
  @Test def testMultipleProposalsInMultipleBands() {
    ...
    verifyTime(
      queueTime(Category.Guaranteed).toHours.value - 10.0,
      queue.remainingTime(Category.Guaranteed)
    )
  }
}
