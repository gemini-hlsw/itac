 ...
 */
class FinalProposalQueue(val queueTime: QueueTime, bandMap: Map[QueueBand, List[Proposal]])
  ...
  def toList: List[Proposal] =
    (QueueBand.values map { band =>
      bandedQueue(band)
    }).flatten

  private def zipBandWithPosition(
    index: Int,
    time: Time,
    band: QueueBand,
    props: List[Proposal]
  ): (List[(Proposal, ProposalPosition)], Int, Time) = {
  ...
  def zipWithPosition: List[(Proposal, ProposalPosition)] = {
    ...
    val (res, _, _) = bandedList.foldLeft((emptyZipped, 0, Time.ZeroHours)) {
      case ((curZipped, startIndex, startTime), (band, propList)) =>
        val (bandZipped, endIndex, endTime) =
          zipBandWithPosition(startIndex, startTime, band, propList)
