...
object RejectOverAllocation {
  ...
  def tooBig(propTime: Time, allRemainingTime: Time) =
    "Adding %.1f hrs for proposal would require more time than remaining in the queue (%.1f hrs)"
      .format(propTime.toHours.value, allRemainingTime.toHours.value)
...
case class RejectOverAllocation(
  prop: Proposal,
  remainingGuaranteedTime: Time,
