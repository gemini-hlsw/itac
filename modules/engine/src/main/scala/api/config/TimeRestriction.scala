 ...
 */
case class TimeRestriction[T](name: String, value: T)(
  val matches: (Proposal, Observation, QueueBand) => Boolean
) {
...
object TimeRestriction {
  ...
  def wv(limit: Percent, wv: WaterVapor) =
    TimeRestriction("WV Queue Time Limit", limit) { (_, obs, _) =>
      obs.conditions.wv <= wv
    }

  def lgs(limit: Time) =
    TimeRestriction("LGS Queue Time Limit", limit) { (_, obs, _) =>
      obs.lgs
    }
