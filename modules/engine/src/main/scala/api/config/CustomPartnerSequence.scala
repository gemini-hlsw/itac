 ...
 *
 */
class CustomPartnerSequence(
  val seq: List[Partner],
  val site: Site,
  val name: String = "Custom Partner Sequence",
  val maybeUseAfterFirstCycle: Option[PartnerSequence] = None,
  val maybePartnerWithInitialPick: Option[Partner] = None
) extends PartnerSequence {
  private val LOGGER = LoggerFactory.getLogger(classOf[CustomPartnerSequence].getName())
  ...
  def configuration: Elem =
    <CustomPartnerSequence name={name} repeating={maybeUseAfterFirstCycle.isEmpty.toString}>
   {seq.map(_.toXML)}
    ...
    if (i + 1 % 100 == 0) {
      ...
      if ((i + 1) % 100000 == 0) {
        ...
        throw new RuntimeException(
          "Queue Engine failed to terminate after 100,000 partner picks. Using partner sequence: " + cycle
        )
      }
    }
