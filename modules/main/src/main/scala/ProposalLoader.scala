...
object ProposalLoader {
  ...
  def apply[F[_]: Sync: Parallel: Logger](
    ...
    new ProposalLoader[F] {
      ...
      def loadPhase1(f:File): F[(I.Proposal, M.Proposal, ReferenceTime)] =
        // if (UpConvert) {
        ...
        Sync[F].delay {
          ...
          editor.applyEdits(f, p).flatMap { rt =>
            Sync[F].delay {
              ...
              (pʹ.copy(observations = pʹ.nonEmptyObservations), p, rt)
            }
          }
        }

      def loadManyPhase1(dir: File): F[List[(File, I.Proposal, M.Proposal, ReferenceTime)]] =
        Sync[F].delay(Option(dir.listFiles)).flatMap {
          case Some(arr) => arr.filter(_.getName().endsWith(".xml")).sortBy(_.getAbsolutePath).toList.traverse(f => loadPhase1(f).map { case (i, m, rt) => (f, i, m, rt) }) // TODO: parTraverse
      ...
      def read(proposal: I.Proposal, mproposal: M.Proposal, p1xml: File, referenceTime: ReferenceTime): EitherNel[String, NonEmptyList[Proposal]] =
        pio.read(proposal, mproposal, referenceTime.getOrElse(defaultReferenceTime), p1xml) match {
      ...
      def load(file: File): F[(File, EitherNel[String, NonEmptyList[Proposal]])] =
        loadPhase1(file).map { case (i, m, rt) => (file, read(i, m, file, rt)) }

      def loadMany(dir: File): F[List[(File, EitherNel[String, NonEmptyList[Proposal]])]] =
        loadManyPhase1(dir).map(_.map(a => (a._1, read(a._2, a._3, a._1, a._4))))
