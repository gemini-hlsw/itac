...
class ProposalMigratorTest {
  ...
  val iqs = Map("20" -> ImageQuality.IQ_20,   "70" -> ImageQuality.IQ_70,   "85" -> ImageQuality.IQ_85,   "Any" -> ImageQuality.IQ_100)
  val sbs = Map("20" -> SkyBackground.SB_20,  "50" -> SkyBackground.SB_50,  "80" -> SkyBackground.SB_80,  "Any" -> SkyBackground.SB_100)
  val wvs = Map("20" -> WaterVapor.WV_20,     "50" -> WaterVapor.WV_50,     "80" -> WaterVapor.WV_80,     "Any" -> WaterVapor.WV_100)
  // fixup mapping: 90->80
  val ccs = Map("50" -> CloudCover.CC_50,     "70" -> CloudCover.CC_70,     "80" -> CloudCover.CC_80,     "90" -> CloudCover.CC_80, "Any" -> CloudCover.CC_100)
...
// /Users/fnussber/Downloads/2011A-xml-only/US/qprop034.xml
  ...
  def collectObservations(builder: ProposalBuilder, proposal: Elem, partner: Partner) = {
  ...
  def collectTargets(proposal: Elem) =
  ...
  def toTarget (t: Node, isToo: Boolean) = {
    ...
    if (isToo) {
      ...
      var c1 = fixUpRaDec(t\"nonSidSystem"\"c1" text) // fixup values
      var c2 = fixUpRaDec(t\"nonSidSystem"\"c2" text) // fixup values
      ...
      var c1 = fixUpRaDec(t\"hmsdegSystem"\"c1" text) // fixup values
      var c2 = fixUpRaDec(t\"hmsdegSystem"\"c2" text) // fixup values
  ...
  def getDefaultCondition(proposal: Elem, conditions: Map[String, Condition]) =
    conditions get (proposal\"observatory"\"obsList"\"constraintRef"\"@constraintId" text) orNull // error handling

  def collectBlueprints(proposal: Elem) = {
  ...
  def toBlueprint(site: String,  r: Node) = {
    site match {
      case "Gemini North"   => {
  ...
  def getDefaultBlueprint(proposal: Elem, blueprints: Map[String, BlueprintBase]) =
    blueprints get (proposal\"observatory"\"obsList"\"resourceRef"\"@resourceId" text) orNull // error handling


  def addObservation (band: Band, o: Node, builder: ProposalBuilder, ctx: Context) = {
  ...
  def collectTac(proposal: Elem, partner: Partner) =
  ...
  def getPartner(submission: Node) : Partner = {
      ...
      case "GeminiStaff"=> ProposalBuilder.dummyPartnerGS
  ...
  def createBuilder(proposal: Elem, partner: Partner) = {
